...
function formatCreated(ms: number, now = new Date()): string {
  let d = new Date(ms)
  ...
  return (
    ...
    >
      ...
      <view flexGrow={1} minWidth={0} paddingTop={10} paddingBottom={10} gap={2} onPointerUp={props.onToggle}>
        ...
        <text fontSize={12} color={MUTED}>{formatCreated(props.todo.created_at)}</text>
      </view>
...
function TodoList(props: { db: Database }) {
  ...
  let rows = createQuery(db, "SELECT id, text, done, created_at FROM todos ORDER BY position, id")
  ...
  let moveToTop = (t: Todo) => {
    let ids = todos().map((x) => x.id).filter((id) => id !== t.id)
    ids.unshift(t.id)
    setLocalOrder(ids)
    saveOrder(ids)
  }
  ...
  let toggle = (t: Todo) => db.run("UPDATE todos SET done = 1 - done WHERE id = ?", [t.id])
  ...
  let lifted = createMemo(() => {
    ...
    let todo = todos().find((t) => t.id === slot.id)
    return todo && { todo, top: slot.top, dy: d.dy }
  })

  return (
    <view flexGrow={1} minHeight={0} gap={14}>
      <view flexDirection="row" alignItems="flex-end" justifyContent="space-between">
        ...
        <view ref={(n: NodeRef) => (content = n)} position="relative" flexShrink={0} gap={GAP} paddingBottom={8}>
          <For each={todos()} keyed={(t) => t.id}>
            {(t) => (
              <Row
                ...
                onMoveToTop={todos()[0]?.id === t().id ? undefined : tap(() => moveToTop(t()))}
                drag={dragFor(t())}
                shift={shiftOf(t().id)}
          ...
          <Show when={todos().length === 0}>
            <view paddingTop={40} alignItems="center">
              <text fontSize={16} color={MUTED}>Nothing to do yet.</text>
            </view>
          </Show>
        </view>
      </view>
      ...
      <Show when={doneCount() > 0}>
        <view alignSelf="center" paddingTop={6} paddingBottom={6} paddingLeft={14} paddingRight={14} onPointerDown={clearDone}>
          <text fontSize={15} color={ACCENT}>Clear {doneCount()} done</text>
        </view>
      </Show>
    </view>
  )
